...
function stashReturnTo(path: string) {
  document.cookie = `${RETURN_TO_COOKIE}=${encodeURIComponent(path)}; Path=/; Max-Age=600; SameSite=Lax`
}

function resolvePostLogin(redirectTo: string, returnTo: string | null): string {
  if (redirectTo === '/dashboard' && isAllowedShareReturnPath(returnTo)) {
    return returnTo
  }
  return redirectTo
}
...
export default function LoginForm({ returnTo }: LoginFormProps) {
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(resolvePostLogin(data.redirectTo, returnTo))
    } catch (error) {
      setMessage({
      ...
      <div className={styles.cardWrapper}>
        ...
        <div className={styles.card}>
          ...
          <p className={styles.subtitle}>
            {returnTo
              ? 'Sign in to open the shared capture.'
